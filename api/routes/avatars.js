...
router.get("/:userID", (req, res) => {
  ...
  const avatarURL = fs.existsSync(
    path.join(__dirname, `../../src/public/avatars/${userID}.png`)
  )
    ? `/avatars/${userID}.png`
    : "/avatars/default.png";
  res.send(avatarURL);
...
router.post("/delete/:userID", async (req, res) => {
  ...
  if (!fs.existsSync(avPath))
    return res.redirect(
      "/me/settings?error=You cannot remove the default avatar"
    );
...
router.post("/createOrReplace", async (req, res) => {
  ...
  if (!req.files)
    return res.redirect(
      "/me/avatar/change?error=Something went wrong. Try again"
    );
  ...
  if (!supportedFileTypes.includes(file.name.split(".").pop()))
    return res.redirect(
      `/me/avatar/change?error=Unsupported file type! Please only use ${supportedFileTypes.join(
        ", "
      )} image types`
    );
  ...
  fs.writeFile(avPath, file.data, (err) => {
    if (err) {
      ...
      console.error(err);
      return res.redirect(
        "/me/settings?error=Failed to change avatar. Please try again"
      );
    }
  });
